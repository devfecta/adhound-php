		...
		if ($Users->Validate($_POST))
		{
			...
			switch($LoginMessage) 
			{
				...
				default:
					...
					if(isset($UserInfo['IA_Users_URL']) && !empty($UserInfo['IA_Users_URL'])) 
					{
						header ("Location: ".$UserInfo['IA_Users_URL']);
					}
					else 
					{
...
<img src="images/AdHound_Logo.gif" style="padding:10px;" border="0" alt="AdHound&trade; - It's Advertising, LLC" />
	...
	<div style="display:inline-block; float:left; width:20%; padding:5px 5px 5px 10px; text-align:right">
		Password:<br />
		<a href="forgot.php" id="ForgotPasswordLink" name="ForgotPasswordLink" style="font-size:9px" title="Forgot AdHound Password">[Forgot password?]</a> 
	</div>
	<div style="display:inline-block; width:auto; padding:5px 5px 5px 10px; text-align:left">
	...
	<div style="display:inline-block; width:auto; padding:5px 5px 5px 10px; white-space:nowrap;">
		<input type="button" id="RegisterButton" name="RegisterButton" onclick="window.location='register.php'" style="width:160px; height:30px; margin-right:50px; float:left" value="Register for a FREE Trial" />
		<input type="submit" id="LoginButton" name="LoginButton" style="width:110px; height:30px; float:right" value="Account Login" /> 
	</div>
