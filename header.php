	...
	if (isset($UserInfo['IA_Users_ID'])) 
	{
		...
		switch ($UserInfo['IA_Users_Type']) 
		{
			case 4:
				break;
			default:
				if((!$UserInfo['ValidCard'] && basename($_SERVER["PHP_SELF"]) != 'account.php') || (!$UserInfo['ValidCard'] && $_SESSION['ModeType'] != "EditUserAccount")) 
				{
					$_SESSION['ModeType'] = "EditUserAccount";
					header ("Location: account.php");
				}
		...
		switch ($UserInfo['IA_Users_Type']) 
		{
			...
			default:
				...
				if ($Users->GetUserInfo($_SESSION['UserID']))
				{
					...
					if((!$Users->ValidCard && basename($_SERVER["PHP_SELF"]) != 'account.php') || (!$Users->ValidCard && $_SESSION['ModeType'] != "EditUserAccount")) 
					{
						$_SESSION['ModeType'] = "EditUserAccount";
						header ("Location: account.php");
					}
...
<div id="Header" style="background-color:#ffffff">
	<div id="Header_Logo">
		<a href="account.php" title="AdHound&trade;"><img src="images/AdHound_Logo.gif" border="0" alt="AdHound&trade; - It's Advertising, LLC" /></a>
		<?php
			/*
			$kilobyte = 1024;
		...
		<div id="Header_Nav_User">
			...
			if (isset($UserInfo['IA_Users_ID'])) 
			{
				if (!empty($FirstName) && !empty($LastName)) 
				{
					...
					echo 'Hello, <a href="account.php" title="My Account" class="HeaderLink">'.$FirstName.' '.$LastName.'</a>';
					switch ($UserInfo['IA_Users_Type']) 
					{
						case 1:
						case 2:
				...
				{
					echo 'Hello, <a href="account.php" title="My Account" class="HeaderLink">'.$BusinessName.'</a>';
				}
			...
			{
				echo '<a href="https://itsadvertising.c6.ixsecure.com/account.php" title="It\'s Advertising, LLC Account Login">Account Login</a>';
			}
			?>
		</div>
		<?php
