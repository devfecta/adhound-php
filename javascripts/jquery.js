...
function CheckUsername(Username) 
{
	...
	{
		$.post("configuration/jquery.php", {FunctionType: "CheckUsername", User: ""+Username+""}, function(data) {
			//document.getElementById('loading').style.display = 'none';
			$('#SearchResults').fadeIn('slow');
			$('#SearchResults').html(data);
		});
...
function ActivateUser(UserInfo)
{
	...
	{
		$.post("configuration/jquery.php", {FunctionType: "ActivateUser", UserID: ""+UserInfo.split('-')[0]+"", Activate: ""+UserInfo.split('-')[1]+""}, function(data) {
...
function ValidateCard(UserInfo)
{
	if(UserInfo.toString().length > 0) 
	{
		$.post("configuration/jquery.php", {FunctionType: "ValidateCard", UserID: ""+UserInfo.split('-')[0]+"", Validate: ""+UserInfo.split('-')[1]+""}, function(data) {
			//$('#ActivateUser').html(data);
			window.location.reload();
		});
	} 
...
function DeleteUser(ParentUserID, UserID) 
{
	...
	if (answer)
	{
		...
		{
			document.getElementById('loading').style.display = 'block';
			$.post("configuration/jquery.php", {FunctionType: "DeleteUser", ParentUser: ""+ParentUserID+"", User: ""+UserID+""}, function(data) {
				document.getElementById('loading').style.display = 'none';
...
function UpdatePanelThumbnail(UserID, UserTypeID, AccountID, PanelsID, PanelID, PanelSectionID, AdvertiserID, AdID)
{
	if(AdID.toString().length > 0) 
	{
		$.post("configuration/jquery.php", {FunctionType: "UpdatePanelThumbnail", User: ""+UserID+"", Account: ""+AccountID+"", Panels: ""+PanelsID+"", Panel: ""+PanelID+"", PanelSection: ""+PanelSectionID+"", Advertiser: ""+AdvertiserID+"", Ad: ""+AdID+""}, function(data) {
			//$('#PanelThumbnailDIV').html(data);
			//$('#PanelCell'+PanelID).html(data);
			//$('#PanelCell').html(data);
			ShowPanelThumbnail(UserTypeID, AccountID, PanelsID, PanelID);
...
function GetAdFiles(UserID, UserTypeID, AdvertiserID, AdID, PanelSectionWidth, PanelWidth, PanelSectionHeight, PanelHeight)
{
	/*
	xmlhttp.onreadystatechange=function()
	{
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			panels=xmlhttp.responseText;
			document.getElementById('AdFilesDIV').innerHTML=xmlhttp.responseText;
		}
	}

	xmlhttp.open("POST","configuration/ajax.php",true);
	xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xmlhttp.send("FunctionType=GetAdFiles"+
				"&AdvertiserID="+AdvertiserID+
				"&AdID="+AdID+
	...
	*/
	if(AdvertiserID.toString().length > 0) 
	{
		$.post("configuration/jquery.php", {FunctionType: "GetAdFiles", User: ""+UserID+"", UserType: ""+UserTypeID+"", Advertiser: ""+AdvertiserID+"", Ad: ""+AdID+"", SectionWidth: ""+PanelSectionWidth+"", PWidth: ""+PanelWidth+"", SectionHeight: ""+PanelSectionHeight+"", PHeight: ""+PanelHeight+""}, function(data) {
			$('#AdFilesDIV').html(data);
		});
	} 
	else 
...
// panels.php
/*
function AddPanelLocation(AccountID)
{
	xmlhttp.open("POST","configuration/ajax.php",true);
	xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xmlhttp.send("FunctionType=AddPanelLocation"+
				"&AccountID="+AccountID+
				"&PanelLocationTextBox="+document.getElementById('PanelLocationTextBox').value);
