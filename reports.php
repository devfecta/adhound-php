<?php
	...
	if(isset($_REQUEST['ReportType']) && !empty($_REQUEST['ReportType'])) 
	{
		...
		switch ($ReportType[0])
		{
			...
			case 'ClientAdListing':
				$PageTitle = $UserInfo['IA_Users_BusinessName'].'\'s '.$PageTitle;
			...
			case 'RegionalRunReport':
				if(!empty($ReportType[1])) 
				{
					...
					$AccountName = $UserInfo['IA_Users_BusinessName'];
					$AccountContact = $UserInfo['IA_Users_FirstName'].' '.$UserInfo['IA_Users_LastName'];
					$AccountAddress = $UserInfo['IA_Users_Address'].'<br />'.$UserInfo['IA_Users_City'].', '.$UserInfo['IA_States_Abbreviation'].' '.$UserInfo['IA_Users_Zipcode'];
					$AccountPhone = $UserInfo['IA_Users_Phone'];
					$AccountNotes = '';
	...
	{
		$PageTitle = $UserInfo['IA_Users_BusinessName'];
	}
...
if (isset($_REQUEST['ReportType']))
{
	...
	*/
	...
	switch ($ReportType[0])
	{
		case 'RegionalRunReport':
		case 'RunReport':
			echo $Reports->BuildRunReport($UserInfo, $ReportType[1], $_REQUEST['AreaID'], $_REQUEST['RoomID'], $_REQUEST['AdLocationID'], $ReportType[0]);
			break;
		case 'ProofOfPerformance':
			/*
		...
		case 'SiteOpenings':
			...
			if ($PanelCount > 0)
			{
				while ($PanelInfo = mysql_fetch_assoc($PanelsInfo))
				{
					echo $Reports->BuildPanelList($UserInfo['IA_Users_Type'], $PanelInfo[IA_Panels_AccountID], $_REQUEST[LocationID], $PanelInfo[IA_Panels_PanelID], $_REQUEST[ModeType]);
				}
			}
			else
			{
