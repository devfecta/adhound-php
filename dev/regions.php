...
<?php 
	...
	foreach($States as $Node => $Value) 
	{
		unset($States[$Node]->Regions->Locations);
	}
	$StateInfo = json_decode(json_encode($States),true);
	echo '<h2 style="margin:0px 0px 5px 0px">';
	if(isset($_REQUEST['ModeType']) && $_REQUEST['ModeType'] == 'EditRegion') 
	{
		echo 'Edit Region';
		$Regions = $Data->xpath('/Data/State/Regions/Region[@id="'.$_REQUEST['RegionID'].'"]');
		$RegionInfo = json_decode(json_encode($Regions),true);
	...
	echo $Accounts->BuildRegionsForm($StateInfo, $RegionInfo, $_REQUEST['ModeType']);
	echo '</div>';
	
	$Regions = $Data->xpath('/Data/State/Regions/Region');
	foreach($Regions as $Node => $Value) 
	{
		unset($Regions[$Node]->Locations);
	}
	$RegionInfo = json_decode(json_encode($Regions),true);
//print("Data<pre>". print_r($Data,true) ."</pre>");
	/*
