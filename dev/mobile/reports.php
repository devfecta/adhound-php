...
//print("Data<pre>". print_r($LocationInfo,true) ."</pre>");
		...
		case 'ContractReport':
			...
			{
				case 'Account':
					$Ads = $Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$ReportType[1].'"]/Panels/Areas/Rooms/Walls/Panel/Ad');
					...
					break;
				case 'Region':
					$Ads = $Data->xpath('/Data/State/Regions/Region[@id="'.$ReportType[1].'"]/Locations/Location/Panels/Areas/Rooms/Walls/Panel/Ad');
					$Ads = json_decode(json_encode($Ads),true);
			...
			if (count($Ads) > 0)
			{
				...
				if(!isset($AccountInfo[0]) && empty($AccountInfo[0])) 
				{
...
//print("Data<pre>". print_r($AccountInfo,true) ."</pre>");
