	...
	class _Panels extends _Validation
	{
		...
		public function AddPanel($UserInfo, $PanelInfo)
		{
			...
			if (mysql_query($InsertPanel, CONN) or die(mysql_error())) 
			{
				...
				$Panels = mysql_query("SELECT * FROM IA_Panels, IA_LocationAreas, IA_LocationRooms, IA_AdLocations, IA_AdPanels WHERE IA_Panels_ID=".$NewID." AND IA_LocationAreas_ID=IA_Panels_AreaID AND IA_LocationRooms_ID=IA_Panels_RoomID AND IA_AdLocations_ID=IA_Panels_LocationID AND IA_AdPanels_ID=IA_Panels_PanelID", CONN);
				while($Panel = mysql_fetch_assoc($Panels))
				{
					...
					if($xPath->evaluate('/Data/State/Regions/Region/Locations/Location[@id="'.$Panel['IA_Panels_AccountID'].'"]/Panels/Areas[@id="'.$Panel['IA_LocationAreas_ID'].'"]/Rooms[@id="'.$Panel['IA_LocationRooms_ID'].'"]/Walls[@id="'.$Panel['IA_AdLocations_ID'].'"]/Panel[@id="'.$Panel['IA_Panels_ID'].'"]')->length == 0) 
					{
						...
						$ParentAds = $XML->createElement('Ads');
						$ParentAds = $NewNode->appendChild($ParentAds);
					}
					break;
				}
		...
		public function DuplicatePanel($UserID, $PanelID, $AdPanelID)
		{
			$Panels = mysql_query("SELECT * FROM IA_Panels WHERE IA_Panels_ID=".$PanelID, CONN);
			while($Panel = mysql_fetch_assoc($Panels))
			{
				$UserInfo['UserParentID'] = $UserID;
				$PanelInfo['AccountID'] = $Panel['IA_Panels_AccountID'];
				$PanelInfo['AreaDropdownRequired'] = $Panel['IA_Panels_AreaID'];
				$PanelInfo['RoomDropdownRequired'] = $Panel['IA_Panels_RoomID'];
				$PanelInfo['LocationDropdownRequired'] = $Panel['IA_Panels_LocationID'];
				$PanelInfo['PanelIDDropdownRequired'] = $AdPanelID;
				$PanelInfo['SectionsHighTextBox'] = $Panel['IA_Panels_High'];
				$PanelInfo['SectionsWideTextBox'] = $Panel['IA_Panels_Wide'];
				$PanelInfo['PanelHeightTextBox'] = $Panel['IA_Panels_Height'];
				$PanelInfo['PanelWidthTextBox'] = $Panel['IA_Panels_Width'];
				$PanelInfo['PanelDescriptionTextBox'] = '';
				break;
			}
			return $this->AddPanel($UserInfo, $PanelInfo);
		}
		...
		public function DeletePanel($UserID, $AccountID, $PanelID) 
		{
			...
			if (mysql_query($DeletePanel, CONN) or die(mysql_error())) 
			{
				...
				*/
				...
				$xPath = new DOMXpath($XML);
				$RootNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls/Panel[@id="'.$PanelID.'"]');
				
				foreach ($RootNodes as $Node) 
				{
					foreach ($Node->childNodes as $PanelNode) 
					{
						if($PanelNode->nodeName == 'IA_Panels_AreaID') 
						{ $AreaID = $PanelNode->nodeValue; }
					
						if($PanelNode->nodeName == 'IA_Panels_RoomID') 
						{ $RoomID = $PanelNode->nodeValue; }
					
						if($PanelNode->nodeName == 'IA_Panels_LocationID') 
						{ $WallID = $PanelNode->nodeValue; }
					}
					$Node->parentNode->removeChild($Node);
				}
				
				if($xPath->evaluate('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]/Rooms[@id="'.$RoomID.'"]/Walls[@id="'.$WallID.'"]/Panel')->length == 0) 
				{
					$WallNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]/Rooms[@id="'.$RoomID.'"]/Walls[@id="'.$WallID.'"]');
					foreach ($WallNodes as $Node) 
					{ $Node->parentNode->removeChild($Node); }
					if($xPath->evaluate('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]/Rooms[@id="'.$RoomID.'"]/Walls')->length == 0) 
					{
						$RoomNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]/Rooms[@id="'.$RoomID.'"]');
						foreach ($RoomNodes as $Node) 
						{ $Node->parentNode->removeChild($Node); }
						if($xPath->evaluate('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]/Rooms')->length == 0) 
						{
							$AreaNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels/Areas[@id="'.$AreaID.'"]');
...
$Confirmation = true;
			...
			$Advertisements->DeletePanelAdXML($UserInfo, $AdInfo);
...
*/	
		public function BuildPanelList($UserInfo, $AccountID, $RoomID)
		{
			...
			$Panels = $Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$AccountID.'"]/Panels');
			//$Panels = $Panels[0];
			$PanelsInfo = json_decode(json_encode($Panels[0]),true);
			
//print("AccountInfo<pre>". print_r($AccountInfo,true) ."</pre>");	
//print("PanelInfo<pre>". print_r($AreaInfo,true) ."</pre>");

			$PanelList .= '<div style="width:100%; display:block">';
			$PanelList .= '<h1>'.$AccountInfo['IA_Accounts_BusinessName'].'\'s Panels</h1>';
			...
			if(isset($AreaInfo[0]) && !empty($AreaInfo[0])) 
			{
				...
				{
					$PanelList .= '<div style="font-size:13px; max-width:100%; display:block; margin-left:10px; border-top:1px solid #bbbbbb">';
					$PanelList .= "\n".$AreaInfo[$a]['IA_LocationAreas_Area'];
					...
					{
						$RoomSection = null;
						$RoomSection .= '<div style="font-size:12px; max-width:100%; display:block; padding-left:10px; border-top:1px solid #cccccc">';
						$RoomSection .= "\n".$RoomInfo[$r]['IA_LocationRooms_Room'];
						
						$WallInfo = null;
						if(!isset($RoomInfo[$r]['Walls'][0])) 
						{ $WallInfo[] = $RoomInfo[$r]['Walls']; }
						else 
						{ $WallInfo = $RoomInfo[$r]['Walls']; }
						
						for($w=0; $w<count($WallInfo); $w++) 
						{
							$RoomSection .= '<div style="font-size:11px; max-width:100%; display:block; padding-left:10px; border-top:1px solid #dddddd">';
							$RoomSection .= "\n".$WallInfo[$w]['IA_AdLocations_Location'].'<br />';
							
							$PanelInfo = null;
							if(!isset($WallInfo[$w]['Panel'][0])) 
							{ $PanelInfo[] = $WallInfo[$w]['Panel']; }
							else 
							{ $PanelInfo = $WallInfo[$w]['Panel']; }

							for($p=0; $p<count($PanelInfo); $p++) 
							{ 
								$RoomSection .= "\n".'<div style="font-size:10px; min-width:100px; display:inline-block; padding-left:10px; border-top:1px solid #eeeeee">';
								$RoomSection .= "\n".'Panel '.$PanelInfo[$p]['IA_AdPanels_Name'];
								$RoomSection .= ' <i>('.$PanelInfo[$p]['IA_Panels_Wide'].'W x '.$PanelInfo[$p]['IA_Panels_High'].'H Ads)</i><br />';
								//$RoomSection .= ' <input type="button" style="font-size:9px;" onclick="window.location=\'panels.php?AccountID='.$PanelInfo[$p]['IA_Panels_AccountID'].'&PanelID='.$PanelInfo[$p]['IA_Panels_ID'].'\'" id="EditPanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" name="EditPanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" value="Edit"> ';
								$RoomSection .= '<select id="PanelIDDropdown'.$PanelInfo[$p]['IA_Panels_ID'].'" name="PanelIDDropdown'.$PanelInfo[$p]['IA_Panels_ID'].'" style="font-size:9px">'."\r\n";
								if(!empty($PanelInfo)) 
								{
									$PanelIDs = mysql_query("SELECT * FROM IA_AdPanels WHERE IA_AdPanels_ID>0 AND IA_AdPanels_ID NOT IN (SELECT IA_Panels_PanelID FROM IA_Panels WHERE IA_Panels_AccountID=".$PanelInfo[$p]['IA_Panels_AccountID']." AND IA_Panels_AreaID=".$PanelInfo[$p]['IA_Panels_AreaID']." AND IA_Panels_RoomID=".$PanelInfo[$p]['IA_Panels_RoomID']." AND IA_Panels_LocationID=".$PanelInfo[$p]['IA_Panels_LocationID'].") ORDER BY IA_AdPanels_Name", CONN) or die(mysql_error());
									while ($PanelID = mysql_fetch_assoc($PanelIDs))
									{
										$RoomSection .= '<option value="'.$PanelID['IA_AdPanels_ID'].'">'.$PanelID['IA_AdPanels_Name'].'</option>'."\r\n";
									}
								}
								else 
								{ }
								$RoomSection .= '</select>';
								$RoomSection .= '<input type="button" style="font-size:9px; height:15px" onclick="DuplicateRunReportPanel('.$UserInfo['UserParentID'].', '.$PanelInfo[$p]['IA_Panels_ID'].', document.getElementById(\'PanelIDDropdown'.$PanelInfo[$p]['IA_Panels_ID'].'\').value)" id="DuplicatePanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" name="DuplicatePanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" value="Duplicate" /> ';
								$RoomSection .= '<input type="button" style="font-size:9px; height:15px" onclick="DeleteRunReportPanel('.$UserInfo['UserParentID'].', '.$PanelInfo[$p]['IA_Panels_AccountID'].', '.$PanelInfo[$p]['IA_Panels_ID'].')" id="DeletePanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" name="DeletePanelButton'.$PanelInfo[$p]['IA_Panels_ID'].'" value="Delete" /> ';
								
								
								$RoomSection .= $this->BuildPanel($UserInfo, $PanelInfo[$p]['IA_Panels_AccountID'], $PanelInfo[$p]['IA_Panels_ID'], null, 'ImageOnly', .05);
								$RoomSection .= "\n".'<br /><i style="font-size:9px">Description: '.$PanelInfo[$p]['IA_Panels_Description'].'</i>';
								$RoomSection .= '</div>';
							}
							$RoomSection .= '</div>';
						}
						...
						if(isset($RoomID) && !empty($RoomID) && $RoomInfo[$r]['IA_LocationRooms_ID'] == $RoomID) 
						{
							$RoomList = $RoomSection."\n";
							break;
						}
						else 
						{ $RoomList .= $RoomSection."\n"; }
			...
			{
				$PanelList .= '<p style="font-style:italic">No Panels</p>';
			}
			$PanelList .= '</div>';
...
*/
		...
		public function UpdatePanelLocation($UserInfo, $PanelInfo) 
		{
			...
			if (mysql_query($Update, CONN) or die(mysql_error())) 
			{
				...
				//$this->GetPanels($UserInfo['UserParentID'], null, $PanelInfo['AccountID'], null);
				//$Confirmation = true;
				$Confirmation = $this->DeletePanel($UserInfo['UserParentID'], $PanelInfo['AccountID'], $PanelInfo['PanelID']);
				$Confirmation = $this->AddPanel($UserInfo, $PanelInfo);
			}
			/*
			$Confirmation = true;
...
//WIP
		...
		public function BuildPanel($UserInfo, $AccountID, $PanelID, $AdID, $Mode, $Scale)
		{
			...
			$Panel = $Data->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls/Panel[@id="'.$PanelID.'"]');
			$Panel = json_decode(json_encode($Panel),true);
			$Panel = $Panel[0];
			
			$Area = $Data->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas[@id="'.$Panel['IA_Panels_AreaID'].'"]/IA_LocationAreas_Area');
			$Area = json_decode(json_encode($Area),true);
			$Panel['IA_LocationAreas_Area'] = $Area[0][0];
			
			$Room = $Data->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms[@id="'.$Panel['IA_Panels_RoomID'].'"]/IA_LocationRooms_Room');
			$Room = json_decode(json_encode($Room),true);
			$Panel['IA_LocationRooms_Room'] = $Room[0][0];
			
			$Wall = $Data->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls[@id="'.$Panel['IA_Panels_LocationID'].'"]/IA_AdLocations_Location');
			$Wall = json_decode(json_encode($Wall),true);
			$Panel['IA_AdLocations_Location'] = $Wall[0][0];
			
			$AdList = $Data->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls/Panel[@id="'.$PanelID.'"]/Ads/Ad');
			$AdList = json_decode(json_encode($AdList),true);
			$AdsList = array();
			foreach ($AdList as $key => $row)
			{ $AdsList[$key] = $row['IA_Ads_PanelSectionID']; }
			array_multisort($AdsList, SORT_ASC, $AdList);
//print("<pre>". print_r($AdList,true) ."</pre>");
			//$AdsList = $Panel['Ads'];
			//	if(!isset($AdsList[0])) 
			//	{ $AdList[] = $AdsList; }
			
//print("<pre>XML:". print_r($Panel,true) ."</pre>");
				...
				for ($Row=1; $Row<=$Panel['IA_Panels_High']; $Row++)
				{
					...
					for ($Cell=1; $Cell<=$Panel['IA_Panels_Wide']; $Cell++)
					{
						...
						if((isset($AdList[$a]['IA_Ads_AdLibraryID']) && !empty($AdList[$a]['IA_Ads_AdLibraryID'])) && $SectionNumber == $AdList[$a]['IA_Ads_PanelSectionID']) 
						{
							...
							$AdLibraryxPath = new DOMXpath($AdLibraryXML);
							$SectionHeight = number_format((($AdLibraryxPath->query('/AdLibrary/Advertiser/Ad[@id="'.$AdList[$a]['IA_Ads_AdLibraryID'].'"]/IA_AdLibrary_Height')->item(0)->nodeValue * 72) * $Scale), 0, '.', '');
							$SectionWidth = number_format((($AdLibraryxPath->query('/AdLibrary/Advertiser/Ad[@id="'.$AdList[$a]['IA_Ads_AdLibraryID'].'"]/IA_AdLibrary_Width')->item(0)->nodeValue * 72) * $Scale), 0, '.', '');
							$AdHeight = $SectionHeight;
							$AdWidth = $SectionWidth;
							
							if($AdID != $AdList[$a]['IA_Ads_ID'] && $Mode == 'ImageOnly' && $UserInfo['Users_Type'] == 4) 
							{
							...
							if ($AdList[$a]['IA_Ads_Placement'] == 1)
							{
								...
								if (file_exists(ROOT.'/users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'BW.jpg')) 
								{
									...
									$PlaceAd = "\t".'<img id="Ad'.$AdList[$a]['IA_Ads_ID'].'" name="Ad'.$AdList[$a]['IA_Ads_ID'].'" onclick="document.getElementById(\'PanelAdOptions'.$AdList[$a]['IA_Ads_ID'].'\').style.display=\'block\'" src="'.DIRECTORY.'/users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'BW.jpg" style="height:'.$AdHeight.'px; width:'.$AdWidth.'px; cursor:pointer" border="0" alt="'.$AdList[$a]['Advertiser']['IA_Advertisers_BusinessName'].'" />'."\n";
								}
								else 
								{
									$PlaceAd = "\t".'<img id="Ad'.$AdList[$a]['IA_Ads_ID'].'" name="Ad'.$AdList[$a]['IA_Ads_ID'].'" src="images/AdHound_Icon.png" style="border:1px solid #eeeeee; height:'.$AdHeight.'px; width:'.$AdWidth.'px; cursor:pointer" border="0" />'."\n";
							...
							else
							{
								if (file_exists(ROOT.'/users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'.jpg')) 
								{
//echo 'TEST:'.$UserInfo['UserParentID'].'-'.$AdList[$a]['IA_Ads_AdLibraryID'];
									if (file_exists(DIRECTORY.'/users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'.jpg')) 
									{
										$PlaceAd = "\t".'<img id="Ad'.$AdList[$a]['IA_Ads_ID'].'" name="Ad'.$AdList[$a]['IA_Ads_ID'].'" onclick="document.getElementById(\'PanelAdOptions'.$AdList[$a]['IA_Ads_ID'].'\').style.display=\'block\'" src="'.DIRECTORY.'/users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'.jpg" style="height:'.$AdHeight.'px; width:'.$AdWidth.'px; cursor:pointer" border="0" alt="'.$AdList[$a]['Advertiser']['IA_Advertisers_BusinessName'].'" />'."\n";
									}
									else 
									{
										//echo 'test';
										$PlaceAd = "\t".'<img id="Ad'.$AdList[$a]['IA_Ads_ID'].'" name="Ad'.$AdList[$a]['IA_Ads_ID'].'" onclick="document.getElementById(\'PanelAdOptions'.$AdList[$a]['IA_Ads_ID'].'\').style.display=\'block\'" src="users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdList[$a]['IA_Ads_AdLibraryID'].'.jpg" style="height:'.$AdHeight.'px; width:'.$AdWidth.'px; cursor:pointer" border="0" alt="'.$AdList[$a]['Advertiser']['IA_Advertisers_BusinessName'].'" />'."\n";
									}
							...
							$PlaceAd .= '<div style="display:block; background-color:#eeeeee; min-height:20px; width:100%; color:#000000; font-weight:normal; font-size:8px">'.$AdList[$a]['Advertiser']['IA_Advertisers_BusinessName'].'</div>'."\n";
							...
							switch ($Mode)
							{
								...
								default:
									if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Advertisements']['EditAds']))	
									{
										...
										$PlaceAd .= '<a onclick="DeleteRunReportAd('.$UserInfo['Users_Type'].', '.$AdList[$a]['IA_Ads_AdvertiserID'].', '.$AdList[$a]['IA_Ads_ID'].', '.$AdList[$a]['IA_Ads_AccountID'].', '.$Panel['IA_Panels_ID'].', '.$AdList[$a]['IA_Ads_PanelSectionID'].', \''.$Mode.'\', '.$Scale.')">[Delete]</a><br />';
										// WIP
