...
*/
		public function GetAdvertisers($UserID, $AdvertiserID) 
		{
			...
			while($Advertiser = mysql_fetch_assoc($Advertisers))
			{
				...
				$Parent->setAttribute("id", $Advertiser['IA_Advertisers_ID']);
...
*/
		public function AddAdvertiser($AdvertiserInfo, $UserID)
		{
			...
			if (mysql_query($Insert, CONN) or die(mysql_error())) 
			{
				...
				$AdvertiserXML = new DOMDocument('1.0', 'UTF-8');
				$AdvertiserXML->preserveWhiteSpace = false;
				$AdvertiserXML->formatOutput = true;
				$AdvertiserXML->load(ROOT.'/users/'.$UserID.'/data/'.$UserID.'_AdvertisersInfo.xml');
				$xPath = new DOMXpath($AdvertiserXML);
				
				$Advertisers = mysql_query("SELECT * FROM IA_Advertisers, IA_States WHERE IA_Advertisers_ID=".$NewRecordID." AND IA_States_ID=IA_Advertisers_StateID ORDER BY IA_Advertisers_BusinessName", CONN);
				while($Advertiser = mysql_fetch_assoc($Advertisers))
				{
					$AdvertisersNode = $xPath->query('/Advertisers');
					if($xPath->evaluate('/Advertisers/Advertiser[@id="'.$NewRecordID.'"]')->length == 0) 
					{
						foreach ($AdvertisersNode as $Node) 
						{
							$NewNode = $AdvertiserXML->createElement("Advertiser");
					 		$NewNode = $Node->appendChild($NewNode);
							$NewNode->setAttribute("id", $Advertiser['IA_Advertisers_ID']);
							foreach($Advertiser as $Name => $Value)
							{
								$NodeName = $AdvertiserXML->createElement($Name);
								$NodeName = $NewNode->appendChild($NodeName);
								$NodeValue = $AdvertiserXML->createTextNode($Value);
								$NodeValue = $NodeName->appendChild($NodeValue);
							}
						}
						
						$ParentPricings = $AdvertiserXML->createElement('Pricings');
						$ParentPricings = $NewNode->appendChild($ParentPricings);
						$Pricings = mysql_query("
						SELECT IA_AdvertiserPricing.*, 
						IF(IA_AdvertiserPricing_LocationID=0, 0, IA_AdLocations_ID) AS IA_AdLocations_ID,
						IF(IA_AdvertiserPricing_LocationID=0, 'All', IA_AdLocations_Location) AS IA_AdLocations_Location, 
						...
						while ($Pricing = mysql_fetch_assoc($Pricings))
						{
							$ParentPricing = $AdvertiserXML->createElement('Pricing');
							$ParentPricing = $ParentPricings->appendChild($ParentPricing);
							foreach($Pricing as $Name => $Value)
								{
									$NodeName = $AdvertiserXML->createElement($Name);
				...
				$AdvertiserXML->save(ROOT.'/users/'.$UserID.'/data/'.$UserID.'_AdvertisersInfo.xml');
				...
				$AdLibraryXML->load(ROOT.'/users/'.$UserID.'/data/'.$UserID.'_AdLibraryInfo.xml');
				$xPath = new DOMXpath($AdLibraryXML);
				
				$AdLibraryNode = $xPath->query('/AdLibrary');
				if($xPath->evaluate('/AdLibrary/Advertiser[@id="'.$NewRecordID.'"]')->length == 0) 
				{
					foreach ($AdLibraryNode as $Node) 
					{
						$NewNode = $AdLibraryXML->createElement("Advertiser");
				 		$NewNode = $Node->appendChild($NewNode);
						$NewNode->setAttribute("id", $NewRecordID);
					}
				}
				$AdLibraryXML->save(ROOT.'/users/'.$UserID.'/data/'.$UserID.'_AdLibraryInfo.xml');
				...
				if (filter_var($AdvertiserInfo['EmailTextBox'], FILTER_VALIDATE_EMAIL)) 
				{
					...
					$EncryptedPassword = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5(trim($AdvertiserInfo['EmailTextBox'])), $Password, MCRYPT_MODE_CBC, md5(md5(trim($AdvertiserInfo['EmailTextBox'])))));
		       		$Insert = "INSERT INTO Users (Users_Username, Users_Password, Users_Type, Users_BusinessName, Users_FirstName, Users_LastName, Users_Address, Users_City, Users_StateID, Users_Zipcode, Users_Phone, Users_Fax, Users_Email) VALUES ";
				    	$Insert .= "(";
				    	$Insert .= "'".trim($AdvertiserInfo['EmailTextBox'])."', ";
		...
		public function UpdateAdvertiser($UserInfo, $AdvertiserInfo)
		{
			...
			{
				$Confirmation = true;
// Update Main Advertiser XML
				$AdvertisersXML = simplexml_load_file(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdvertisersInfo.xml');
				$AdvertiserNode = $AdvertisersXML->xpath('/Advertisers/Advertiser[@id="'.$AdvertiserInfo['AdvertiserID'].'"]');
				foreach($AdvertiserNode as $a) 
				{
					$a->IA_Advertisers_BusinessName = trim($AdvertiserInfo['BusinessNameTextBoxRequired']);
					$a->IA_Advertisers_FirstName = trim($AdvertiserInfo['FirstNameTextBox']);
					$a->IA_Advertisers_LastName = trim($AdvertiserInfo['LastNameTextBox']);
					$a->IA_Advertisers_Address = trim($AdvertiserInfo['AddressTextBox']);
					$a->IA_Advertisers_City = trim($AdvertiserInfo['CityTextBox']);
					$a->IA_Advertisers_StateID = trim($AdvertiserInfo['StateDropdownRequired']);
					$a->IA_Advertisers_Zipcode = trim($AdvertiserInfo['ZipcodeTextBox']);
					$a->IA_Advertisers_Phone = trim($AdvertiserInfo['PhoneTextBox']);
					$a->IA_Advertisers_Fax = trim($AdvertiserInfo['FaxTextBox']);
					$a->IA_Advertisers_Email = trim($AdvertiserInfo['EmailTextBox']); 
					$a->IA_Advertisers_StartDate = trim($AdvertiserInfo['StartYearDropdownRequired']).'-'.trim($AdvertiserInfo['StartMonthDropdownRequired']).'-'.trim($AdvertiserInfo['StartDayDropdownRequired']);
					$a->IA_Advertisers_ExpirationDate = trim($AdvertiserInfo['ExpireYearDropdownRequired']).'-'.trim($AdvertiserInfo['ExpireMonthDropdownRequired']).'-'.trim($AdvertiserInfo['ExpireDayDropdownRequired']);
					$a->IA_Advertisers_DateDependent = trim($AdvertiserInfo['DateDependentCheckbox']);
					if(isset($AdvertiserInfo['ApplyRentCheckbox']))
					{ $a->IA_Advertisers_ApplyToRent = trim($AdvertiserInfo['ApplyRentCheckbox']); }
					else
					{ $a->IA_Advertisers_ApplyToRent = "0"; }
					
					if (isset($AdvertiserInfo['TaxIDTextBox']) && !empty($AdvertiserInfo['TaxIDTextBox'])) 
					{ $a->IA_Advertisers_TaxID = trim($AdvertiserInfo['TaxIDTextBox']); }
					$a->IA_Advertisers_DateModified = date("Y-m-d h:i:s");
					/*
				...
				$AdvertisersXML->saveXML(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdvertisersInfo.xml');
// Update Ad Library Advertiser XML
				$AdLibraryXML = simplexml_load_file(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdLibraryInfo.xml');
				$AdLibraryNode = $AdLibraryXML->xpath('/AdLibrary/Advertiser[@id="'.$AdvertiserInfo['AdvertiserID'].'"]');
				foreach($AdLibraryNode as $a) 
				{
					$a->IA_Advertisers_BusinessName = trim($AdvertiserInfo['BusinessNameTextBoxRequired']);
					$a->IA_Advertisers_FirstName = trim($AdvertiserInfo['FirstNameTextBox']);
					$a->IA_Advertisers_LastName = trim($AdvertiserInfo['LastNameTextBox']);
				...
				$AdLibraryXML->saveXML(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdLibraryInfo.xml');
				//$this->GetAdvertisers($UserInfo['UserParentID'], null);
// Update Ads Advertiser XML
				$AdsXML = simplexml_load_file(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_Data.xml');
				$AdNode = $AdsXML->xpath('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls/Panel/Ads/Ad/Advertiser[@id="'.$AdvertiserInfo['AdvertiserID'].'"]');
				foreach($AdNode as $a) 
				{
					$a->IA_Advertisers_BusinessName = trim($AdvertiserInfo['BusinessNameTextBoxRequired']);
					$a->IA_Advertisers_FirstName = trim($AdvertiserInfo['FirstNameTextBox']);
				...
				$AdsXML->saveXML(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_Data.xml');
				...
				if (mysql_query($Update, CONN) or die(mysql_error())) 
				{
// Update Advertiser Info for Ads and Ad Library
				...
				if ($OldEmailAddress != $AdvertiserInfo['EmailTextBox']) 
				{
					if (filter_var($AdvertiserInfo['EmailTextBox'], FILTER_VALIDATE_EMAIL)) 
					{
						...
						$Update .= " Users_Username='".trim($AdvertiserInfo['EmailTextBox']);
						$Update .= "', Users_Password='".$EncryptedPassword;
						$Update .= "', Users_Email='".trim($AdvertiserInfo['EmailTextBox']);
						$Update .= "' WHERE Users_Username='".$OldEmailAddress."' AND Users_BusinessName='".$OldBusinessName."'";
		...
		public function DeleteAdvertiser($UserInfo, $AdvertiserID)
		{
			...
			{
				//$this->GetAdvertisers($UserInfo['UserParentID'], null);
				// Remove Advertiser Info from XML
				$XML = new DOMDocument('1.0', 'UTF-8');
				...
				$AdvertiserNodes = $xPath->query('/Advertisers/Advertiser[@id='.$AdvertiserID.']');
				foreach ($AdvertiserNodes as $Node) 
				{
					$Node->parentNode->removeChild($Node);
				}
				$XML->save(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdvertisersInfo.xml');
				
				// Delete Advertiser Pricing
				$Delete = 'DELETE FROM IA_AdvertiserPricing WHERE IA_AdvertiserPricing_AdvertiserID='.$AdvertiserID;
				if (mysql_query($Delete, CONN) or die(mysql_error())) 
				{ $Confirmation = true; }
				
				// Delete Advertiser Placed Ads
				$Delete = 'DELETE FROM IA_Ads WHERE IA_Ads_AdvertiserID='.$AdvertiserID;
				if (mysql_query($Delete, CONN) or die(mysql_error())) 
				{
					// Remove Advertiser Placed Ads from XML
					$XML = new DOMDocument('1.0', 'UTF-8');
					$XML->preserveWhiteSpace = false;
					$XML->formatOutput = true;
					...
					// Modifiable lookup in XML the place to put new data
					$xPath = new DOMXpath($XML);
					$AdNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location/Panels/Areas/Rooms/Walls/Panel/Ads/Ad[IA_Ads_AdvertiserID='.$AdvertiserID.']');
					foreach ($AdNodes as $Node) 
					{
						$Node->parentNode->removeChild($Node);
					}
					$XML->save(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_Data.xml');
					// Delete Advertiser Ad Files
					...
					// Delete Advertiser Ads from Ad Library
					$Delete = 'DELETE FROM IA_AdLibrary WHERE IA_AdLibrary_AdvertiserID='.$AdvertiserID;
					if (mysql_query($Delete, CONN) or die(mysql_error())) 
					{
						$Confirmation = true;
						// Remove Advertiser Ads from Ad Library XML
						$XML = new DOMDocument('1.0', 'UTF-8');
						$XML->preserveWhiteSpace = false;
						$XML->formatOutput = true;
						...
						// Modifiable lookup in XML the place to put new data
						$xPath = new DOMXpath($XML);
						$AdNodes = $xPath->query('/AdLibrary/Advertiser[@id='.$AdvertiserID.']');
						foreach ($AdNodes as $Node) 
						{
							$Node->parentNode->removeChild($Node);
						}
...
//print("PricingInfo<pre>". print_r($PricingInfo,true) ."</pre>");
				...
				*/
				...
				$AdvertiserList .= '<tr style="vertical-align:middle">';
				$AdvertiserList .= '<td style="text-align:right" title="Use the advertiser\'s contract dates instead of the selected dates in the POP and rent reports.">Contract Date Dependent:</td><td colspan="3">';
				...
				$AdvertiserList .= '<tr style="vertical-align:middle">';
				$AdvertiserList .= '<td style="text-align:right" title="Apply this advertiser\'s ads to the rent report.">Apply to Rent:</td><td colspan="3">';
...
//print("AdSizes<pre>". print_r($AdSizes,true) ."</pre>");
						...
						$AdvertiserList .= '<input type="button" id="AddPricingButton" name="AddPricingButton" onclick="AddAdvertiserPricing('.$UserInfo['UserParentID'].', '.$UserInfo['Users_Type'].', '.$AdvertiserID.', document.getElementById(\'AdLocationsDropdown\').value, document.getElementById(\'AdTypeDropdown\').value, document.getElementById(\'AdSizeDropdown\').value, document.getElementById(\'AdCountTextBox\').value, document.getElementById(\'PricingTextBox\').value, document.getElementById(\'PricingIncrementDropdown\').value, document.getElementById(\'StartYearDropdown\').value+\'-\'+document.getElementById(\'StartMonthDropdown\').value+\'-\'+document.getElementById(\'StartDayDropdown\').value, document.getElementById(\'ExpireYearDropdown\').value+\'-\'+document.getElementById(\'ExpireMonthDropdown\').value+\'-\'+document.getElementById(\'ExpireDayDropdown\').value)" value="Add Pricing"> ';
						$AdvertiserList .= '</td>';
						$AdvertiserList .= '</tr>';
						...
						$AdvertiserList .= $this->BuildAdvertiserPricing($UserInfo['UserParentID'], $UserInfo['Users_Type'], $PricingInfo, null);
						//$AdvertiserList .= $this->BuildAdvertiserPricing($UserInfo['UserParentID'], $UserInfo['Users_Type'], $AdvertiserID, null);
						$AdvertiserList .= '</div>'."\n";
					}
					else 
					{
