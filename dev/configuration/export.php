...
switch($_REQUEST['ReportType']) 
{
	case 'RentReport':
		...
		{
			case 'Account':
				$Reports->ContractRentReport($_REQUEST['UserID'], $_REQUEST['ReportView'], $_REQUEST['AccountID'], $_REQUEST['StartDate'], $_REQUEST['EndDate']);
...
//print("<pre>". print_r($Reports->RentReportRowsArray,true) ."</pre>");
			...
			case 'Region':
				...
				$RegionalAds = $Data->xpath('/Data/State/Regions/Region[@id="'.$_REQUEST['RegionID'].'"]/Locations/Location/Panels/Areas/Rooms/Walls/Panel/Ads/Ad');
				$RegionalAds = json_decode(json_encode($RegionalAds),true);
				foreach ($RegionalAds as $AdArray)
				{
				...
				while ($Account = mysql_fetch_assoc($Accounts))
				{
					...
					$Reports->ContractRentReport($_REQUEST['UserID'], $_REQUEST['ReportView'], $Account['IA_Ads_AccountID'], $_REQUEST['StartDate'], $_REQUEST['EndDate']);
					$RentReportArray[] = $Reports->RentReportRowsArray;
				}
...
print("<pre>". print_r($RentReportArray,true) ."</pre>");
		...
		{
			if (!file_exists(ROOT.'/users/'.$_REQUEST['UserID'].'/reports')) 
			{
				mkdir(ROOT.'/users/'.$_REQUEST['UserID'].'/reports', 0777, true);
			}
			else 
			{ }
			...
			{
				$file = fopen(ROOT.'/users/'.$_REQUEST['UserID'].'/reports/'. mysql_insert_id() .'_'.$FileName.'.xls',"w");
