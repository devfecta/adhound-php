	...
	case 'GetContractRentReport':
		...
		switch($_POST['View'])
		{
			...
			case 'Region':
				...
				$Data = simplexml_load_file(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
				$RegionalAds = $Data->xpath('/Data/State/Regions/Region[@id="'.$_POST['Region'].'"]/Locations/Location/Panels/Areas/Rooms/Walls/Panel/Ads/Ad');
				$RegionalAds = json_decode(json_encode($RegionalAds),true);
				foreach ($RegionalAds as $RegionalAdsKey => $AdsArray)
				{
					$Accounts[$RegionalAds[$RegionalAdsKey]['IA_Ads_AccountID']] = $RegionalAds[$RegionalAdsKey]['IA_Ads_AccountID'];
				}
				$Accounts = array_values($Accounts);
				foreach ($Accounts as $AccountID)
				{
					$Reports->ContractRentReport($_POST['User'], $_POST['View'], $AccountID, $_POST['Start'], $_POST['End']);
					echo $Reports->RentReport;
				}
...
*/
	...
	case 'UpdatePanelThumbnail':
		...
		$AdInfo['IA_Ads_AdLibraryID'] = $_POST['AdLibrary'];
		//$AdInfo['IA_Ads_PanelsID'] = $_POST['Panel'];
		$AdInfo['IA_Ads_PanelSectionID'] = $_POST['PanelSection'];
		$AdInfo['IA_Ads_AccountID'] = $_POST['Account'];
		
		$AdInfo['AccountDropdownRequired'] = $_POST['Account'];
		$AdInfo['PanelLocationDropdown'] = $_POST['Panels'];
		
		$Advertisements = new _Advertisements();
		$Confirmation = $Advertisements->UpdatePanelAdXML($UserInfo, $AdInfo);
		...
		if(!empty($_POST['Panel']) && !empty($_POST['PanelSection']) && !empty($Panel[0]) && !empty($_POST['Account'])) 
		{
			...
			else 
			{
				...
				$Update .= ' IA_Ads_AdLibraryID='.$_POST['AdLibrary'];
				$Update .= ', IA_Ads_AdvertiserID='.$_POST['Advertiser'];
				$Update .= ' WHERE IA_Ads_ID='.$_POST['Ad'];
			...
			if (mysql_query($Update, CONN) or die(mysql_error())) 
			{
				if ($_POST['Ad'] != null)
				{
					...
					$XML->load(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
					
					$xPath = new DOMXpath($XML);
					$PanelNode = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$_POST['Account'].'"]/Panels/Areas/Rooms/Walls/Panel[@id="'.$Panel[0].'"]');
					
					$Ads = mysql_query("SELECT * FROM IA_Ads WHERE IA_Ads_ID=".$NewID, CONN);
					while($Ad = mysql_fetch_assoc($Ads))
					{
						foreach ($PanelNode as $Node) 
						{
							$AdNode = $XML->createElement("Ad");
			    				$AdNode = $Node->appendChild($AdNode);
							$AdNode->setAttribute("id", $Ad['IA_Ads_ID']);
						}
						...
						$Advertisers = mysql_query("SELECT * FROM IA_Advertisers WHERE IA_Advertisers_ID=".$Ad['IA_Ads_AdvertiserID'], CONN);
						while($Advertiser = mysql_fetch_assoc($Advertisers))
						{
							$AdvertiserNode = $XML->createElement("Advertiser");
			    				$AdvertiserNode = $AdNode->appendChild($AdvertiserNode);
					...
					$XML->save(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
					echo $NewID;
				}
				else 
				{
					
					// Delete Advertiser Info for the Ad
					$XML = new DOMDocument('1.0', 'UTF-8');
					$XML->preserveWhiteSpace = false;
					$XML->formatOutput = true;
					$XML->load(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
					...
					$xPath = new DOMXpath($XML);
					$RootNodes = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$_POST['Account'].'"]/Panels/Areas/Rooms/Walls/Panel[@id="'.$Panel[0].'"]/Ad[@id="'.$_POST['Ad'].'"]/Advertiser');
					foreach ($RootNodes as $Node) 
					{
						$Node->parentNode->removeChild($Node);
					}
					
					$AdNode = $xPath->query('/Data/State/Regions/Region/Locations/Location[@id="'.$_POST['Account'].'"]/Panels/Areas/Rooms/Walls/Panel[@id="'.$Panel[0].'"]/Ad[@id="'.$_POST['Ad'].'"]');
					
					$Advertisers = mysql_query("SELECT * FROM IA_Advertisers WHERE IA_Advertisers_ID=".$_POST['Advertiser'], CONN);
					while($Advertiser = mysql_fetch_assoc($Advertisers))
					{
						$AdvertiserNode = $XML->createElement("Advertiser");
		    				$AdvertiserNode = $AdNode->appendChild($AdvertiserNode);
						$AdvertiserNode->setAttribute("id", $Ad['IA_Advertisers_ID']);
						foreach($Advertiser as $Name => $Value)
						{
							$NodeName = $XML->createElement($Name);
							$NodeName = $AdvertiserNode->appendChild($NodeName);
					...
					$Data = simplexml_load_file(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
					foreach($Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$_POST['Account'].'"]/Panels/Areas/Rooms/Walls/Panel[@id="'.$Panel[0].'"]/Ad[@id="'.$_POST['Ad'].'"]') as $a ) 
					{
						$a->IA_Ads_AdLibraryID = $_POST['AdLibrary'];
						$a->IA_Ads_AdvertiserID = $_POST['Advertiser'];
					}
					$Data->saveXML(ROOT.'/users/'.$_POST['User'].'/data/'.$_POST['User'].'_Data.xml');
					echo $_POST['Ad'];
				}
	...
	case 'GetAdFiles':
		...
		*/
		...
		echo '</tr>'."\n";
		*/
		$Data = simplexml_load_file(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_AdLibraryInfo.xml');
		$AdInfo = $Data->xpath('/AdLibrary/Advertiser[@id='.$_POST['Advertiser'].']/Ad[IA_AdLibrary_Height<='.$_POST['PHeight'].' or IA_AdLibrary_Height<='.$_POST['SectionHeight'].' and IA_AdLibrary_Width<='.$_POST['PWidth'].' or IA_AdLibrary_Width<='.$_POST['SectionWidth'].']');
		...
		*/
		...
		for($a=0; $a<count($AdInfo); $a++) 
		{
			...
			if ($AdInfo[$a]['IA_AdLibrary_ID'] == $_POST['AdLibrary'] && $AdInfo[$a]['IA_AdLibrary_Archived'] == 0)
			{
				echo '<input type="radio" id="SelectedAdRadioButton" name="SelectedAdRadioButton" onclick="UpdatePanelThumbnail('.$UserInfo['UserParentID'].', '.$UserInfo['Users_Type'].', document.getElementById(\'AccountDropdownRequired\').value, document.getElementById(\'PanelLocationDropdown\').value, document.getElementById(\'PanelIDDropdownRequired\').value, document.getElementById(\'PanelSectionDropdownRequired\').value, '.$AdInfo[$a]['IA_Advertisers_ID'].', '.$_POST['Ad'].', '.$AdInfo[$a]['IA_AdLibrary_ID'].')" value="'.$AdInfo[$a]['IA_AdLibrary_ID'].'" checked="true" />';
				/*
				echo '<tr><td style="width:5%; vertical-align:middle; border-bottom:1px solid #000000">';
				echo '<label style="white-space:nowrap"><input type="radio" id="SelectedAdRadioButton" name="SelectedAdRadioButton" onclick="UpdatePanelThumbnail('.$UserInfo['UserParentID'].', '.$UserInfo['Users_Type'].', document.getElementById(\'AccountDropdownRequired\').value, document.getElementById(\'PanelLocationDropdown\').value, document.getElementById(\'LocationDropdown\').value, document.getElementById(\'PanelIDDropdownRequired\').value, document.getElementById(\'PanelSectionDropdownRequired\').value, '.$AdInfo[$a]['IA_Advertisers_ID'].', '.$AdInfo[$a]['IA_AdLibrary_ID'].')" value="'.$AdInfo[$a]['IA_AdLibrary_ID'].'" checked="true" />'.'<img src="users/'.$UserInfo['UserParentID'].'/images/lowres/ad'.$AdInfo[$a]['IA_AdLibrary_ID'].'.jpg" style="width:'.(($AdInfo[$a]['IA_AdLibrary_Width'] * 72) * .1).'px; height:'.(($AdInfo[$a]['IA_AdLibrary_Height'] * 72) * .1).'px" border="0" alt="'.$AdInfo[$a]['IA_Advertisers_BusinessName'].'" /></label>';
				echo '</td>'."\n";
...
*/
	...
	case 'DeleteRegion':
		$Accounts = new _Accounts();
		if($Accounts->DeleteRegion($_POST['User'], $_POST['Region'])) 
		{ }
		break;
	case 'GetRegions':
		...
		{
			if($_POST['State'] == $Regions[$r]['IA_Regions_StateID']) 
			{
				echo '<option value="'.$Regions[$r]['IA_Regions_ID'].'">'.$Regions[$r]['IA_Regions_Name'].'</option>';
			}
		}
		/*
		$Regions = mysql_query("SELECT * FROM IA_Regions WHERE IA_Regions_StateID=".$_POST['State']." AND IA_Regions_UserID=".$_POST['User']." ORDER BY IA_Regions_Name", CONN);
	...
	case 'DeleteLocation':
		...
		{ }
		break;
	case 'DuplicateRunReportPanel':
		$Panels = new _Panels();
		if ($Panels->DuplicatePanel($_POST['User'], $_POST['Panel'], $_POST['AdPanel']))
		{ }
		else
		{ }
		break;
	...
	case 'AdvertiserSearch':
		...
		for($a=0; $a<count($AdvertiserInfo); $a++) 
		{
			...
			if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Advertisers']['EditAdvertisers']))	
			{
			...
			if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Advertisers']['ViewPOPReports']))	
			{
		...
		break;
	case 'LocationSearch':
		$Data = simplexml_load_file(ROOT.'/users/'.$_POST['UserID'].'/data/'.$_POST['UserID'].'_Data.xml');
		$Locations = $Data->xpath('/Data/State/Regions/Region/Locations/Location');
		$Locations = json_decode(json_encode($Locations),true);
		
		switch($_POST['Mode']) 
		{
			case 'City':
				$CityState = explode(", ", trim($_POST['QueryString']));
				break;
			default:
				$CityState = array(0=>'xxx', 1=>'xxx');
				break;
		}
	
		foreach($Locations as $Location) 
		{
			if(preg_match('/'. trim($_POST['QueryString']).'/i', $Location['IA_Accounts_BusinessName']) || preg_match('/'. trim($_POST['QueryString']).'/i', $Location['IA_Accounts_City']) || (preg_match('/'. $CityState[0] .'/i', $Location['IA_Accounts_City']) && preg_match('/'. $CityState[1] .'/i', $Location['IA_States_Abbreviation'])) || preg_match('/'. trim($_POST['QueryString']) .'/i', $Location['IA_Counties_Name']) || preg_match('/'. trim($_POST['QueryString']) .'/i', $Location['IA_Accounts_Zipcode']) || preg_match('/'. trim($_POST['QueryString']) .'/i', $Location['IA_Regions_Name'])) 
			{ $LocationInfo[] = $Location; }
		}
		
		if(count($LocationInfo) > 0) 
		{ 
			$Results = '<p style="font-style:italic">Number of Locations Found: '. count($LocationInfo);
			$Results .= ' <input type="button" onclick="location.reload();" id="ClearResults" name="ClearResults" value="Clear Search Results">';
			$Results .= '</p>';
		}
		else 
		{ $Results = '<p style="font-style:italic">No Locations Found</p>'; }
	
		$RegionInfo = $Data->xpath('/Data/State/Regions/Region');
		$RegionInfo = json_decode(json_encode($RegionInfo),true);

		for($r=0; $r<count($RegionInfo); $r++) 
		{
			$Region = null;
			// Region Row
			$Region .= '<div id="AccountListRegionRow" id="AccountListRegionRow">';
			$Region .= $RegionInfo[$r]['IA_Regions_Name'];
			$Region .= ' <select id="ViewReportDropdown" name="ViewReportDropdown" style="margin-bottom:3px;" onchange="window.location=this.options[this.selectedIndex].value;">';
			$Region .= '<option value="">Select a Regional Option</option>';
			if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Locations']['ViewRunReports']))	
			{
				$Region .= '<option value="reports.php?ReportType=RegionalRunReport+'.$RegionInfo[$r]['IA_Regions_ID'].'">Regional Run Report</option>';
			}
			if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Locations']['ViewRentReports']))	
			{
				$Region .= '<option id="RentReport" name="RentReport" value="reports.php?ReportType=ContractReport+'.$RegionInfo[$r]['IA_Regions_ID'].'&ReportView=Region">Regional Rent Report</option>';
			}							
			//$Region .= '<option id="AdCount" name="AdCount" value="reports.php?ReportType=AdSummary+'.$RegionInfo[$r]['IA_Regions_ID'].'&ReportView=Region">Regional Ad Count</option>';
			$Region .= '</select>';
			$Region .= '</div>';
				...
				{
					if($RegionInfo[$r]['IA_Regions_ID'] == $LocationInfo[$l]['IA_Accounts_RegionID']) 
					{
						// Location Row
						if ($RowCount == 0)
						{
							$Location .= '<div id="AccountListAccountRow'.$LocationInfo[$l]['IA_Accounts_ID'].'" style="width:auto; min-height:50px; padding:5px 0px 5px 10px; vertical-align:middle; display:block; background: url(images/table_background.png) repeat-x">';
							$RowCount = 1;
						}
						else
						{
							$Location .= '<div id="AccountListAccountRow'.$LocationInfo[$l]['IA_Accounts_ID'].'" style="width:auto; min-height:50px; padding:5px 0px 5px 10px; vertical-align:middle; display:block; background: url(images/table_background.png) repeat-x; background-color:#eeeeee">';
							$RowCount = 0;
						}
						
						$Location .= '<div id="AccountListColumn1">';
							$Location .= '<b>'.$LocationInfo[$l]['IA_Accounts_BusinessName'].'</b><br />';
							$Location .= '<a href="';
							$Location .= 'http://maps.google.com/maps?q='.str_replace(" ", "+", $LocationInfo[$l]['IA_Accounts_Address']).','.str_replace(" ", "+", $LocationInfo[$l]['IA_Accounts_City']).','.$LocationInfo[$l]['IA_States_Abbreviation'].','.$LocationInfo[$l]['IA_Accounts_Zipcode'].'&hl=en&z=15" target="_blank">';
							$Location .= $LocationInfo[$l]['IA_Accounts_Address'];
							$Location .= '</a><br />';
							$Location .= '<a href="';
							$Location .= 'http://maps.google.com/maps?q='.str_replace(" ", "+", $LocationInfo[$l]['IA_Accounts_City']).','.$LocationInfo[$l]['IA_States_Abbreviation'].','.$LocationInfo[$l]['IA_Accounts_Zipcode'].'&hl=en&z=9" target="_blank">';
							$Location .= $LocationInfo[$l]['IA_Accounts_City'];
							$Location .= '</a>';
							$Location .= ', '.$LocationInfo[$l]['IA_Counties_Name'];
							$Location .= ', '.$LocationInfo[$l]['IA_States_Abbreviation'];
							$Location .= ' '.$LocationInfo[$l]['IA_Accounts_Zipcode'].'<br />';
							$Location .= 'Phone: '.$LocationInfo[$l]['IA_Accounts_Phone'];
							$Location .= ' Fax: '.$LocationInfo[$l]['IA_Accounts_Fax'].'<br />';
							$Location .= '<a href="mailto:'.$LocationInfo[$l]['IA_Accounts_Email'].'">'.$LocationInfo[$l]['IA_Accounts_Email'].'</a>';
							$Location .= '</div>'."\n";
							...
							if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Locations']['ViewRunReports']))	
							{
								$Location .= '<option value="reports.php?ReportType=RunReport+'.$LocationInfo[$l]['IA_Accounts_ID'].'">Run Report</option>';
							}
							if($UserInfo['Users_Type'] == 1 || isset($UserInfo['Preferences']['Locations']['ViewRentReports']))	
							{
								$Location .= '<option id="RentReport" value="reports.php?ReportType=ContractReport+'.$LocationInfo[$l]['IA_Accounts_ID'].'&ReportView=Account">Rent Report</option>';
								//$Location .= '<option id="AdCount" value="reports.php?ReportType=SiteOpenings+'.$LocationInfo[$l]['IA_Accounts_ID'].'">Site Openings</option>';
							}
							//$Location .= '<option value="reports.php?ReportType=AdSummary+'.$LocationInfo[$l]['IA_Accounts_ID'].'&ReportView=Account">Ad Count</option>';
							$Location .= '</select><br />';
							
							$AdNodes = $Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$LocationInfo[$l]['IA_Accounts_ID'].'"]/Panels/Areas/Rooms/Walls/Panel/Ads/Ad');
							$AdNodes = json_decode(json_encode($AdNodes),true);
							$Location .= 'Ad Count: '. count($AdNodes);
							...
							{
								$Location .= '<input type="button" style="font-size:11px" id="EditAccountButton'.$LocationInfo[$l]['IA_Accounts_ID'].'" name="EditAccountButton'.$LocationInfo[$l]['IA_Accounts_ID'].'" onclick="window.location=\'locations.php?AccountID='.$LocationInfo[$l]['IA_Accounts_ID'].'&ModeType=EditAccount\'" value="Edit Location"> ';
								// Work on later
								...
								//}
								$Location .= '&nbsp;<input type="button" style="font-size:11px" id="DeleteAccountButton'.$LocationInfo[$l]['IA_Accounts_ID'].'" name="DeleteAccountButton'.$LocationInfo[$l]['IA_Accounts_ID'].'" onclick="DeleteLocation('.$LocationInfo[$l]['IA_Accounts_ID'].', null)" value="Delete Location"><br /><br />';
							}
		...
		if (mysql_query($Update, CONN) or die(mysql_error())) 
		{
			switch ($_POST['FunctionType'])
			{
				case 'AddDamage':
					...
					$xPath = new DOMXpath($XML);
					$DamageLogsNode = $xPath->query('/Data/State/Regions/Region/Locations/Location/DamageLogs[@id="'.$_POST['Account'].'"]');
					
					$DamageLogs = mysql_query("SELECT * FROM IA_LocationDamageLog WHERE IA_LocationDamageLog_ID=".$NewID, CONN);
					while($DamageLog = mysql_fetch_assoc($DamageLogs))
					{
						foreach($DamageLogsNode as $Node)
						{
							$DamageLogNode = $XML->createElement("DamageLog");
			    			$DamageLogNode = $Node->appendChild($DamageLogNode);
						}
						foreach($DamageLog as $Name => $Value)
						{
							$NodeName = $XML->createElement($Name);
							$NodeName = $DamageLogNode->appendChild($NodeName);
							$NodeValue = $XML->createTextNode($Value);
							$NodeValue = $NodeName->appendChild($NodeValue);
						}
						break;
				...
				default:
					...
					foreach($Data->xpath('/Data/State/Regions/Region/Locations/Location/DamageLogs[@id="'.$_POST['Account'].'"]/DamageLog') as $d ) 
					{
						if($d->IA_LocationDamageLog_ID == $_POST['Log']) 
						{
							$d->IA_LocationDamageLog_Fixed = $_POST['Fixed'];
							$d->IA_LocationDamageLog_TimeStamp = date("Y-m-d H:i:s");
							break;
						}
					}
			...
			$Data = simplexml_load_file(ROOT.'/users/'.$UserInfo['UserParentID'].'/data/'.$UserInfo['UserParentID'].'_Data.xml');
			$Locations = $Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$_POST['Account'].'"]');
			...
			*/
			if(isset($LocationInfo['DamageLogs']['DamageLog']) && !empty($LocationInfo['DamageLogs']['DamageLog'])) 
			{
