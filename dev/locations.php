...
/*
		foreach($Data->xpath('/Data/State/Regions/Region/Locations/Location[@id="'.$_REQUEST['AccountID'].'"]') as $l ) 
		{
			echo 'test2';
			foreach($l->xpath('RentTerms') as $r) 
			{
				echo 'test3'.$r->IA_PaymentIncrements_Increment;
			}
		}
*/
		if(isset($_REQUEST['AccountID']) && !empty($_REQUEST['AccountID'])) 
		{
			...
			{
				if($Location['IA_Accounts_ID'] == $_REQUEST['AccountID']) 
				{
					$LocationInfo = null;					
...
//print("Data<pre>". print_r($LocationInfo,true) ."</pre>");
		...
		if ($_REQUEST['ModeType'] == 'AddAccount' || $_REQUEST['ModeType'] == 'EditAccount')
		{
			...
			*/
			echo $Accounts->BuildAccountForm($UserInfo, $LocationInfo, $RegionInfo, $_REQUEST['ModeType']);
		}
		else
		{
